...
test("nutrition home keeps the approved six screens in sequence", async ({ page }) => {
  ...
  await expect(capsuleScience).toHaveAttribute("data-static-design", "screen-04-approved");
  await expect(capsuleScience.getByRole("heading", { name: "Precision inside every capsule." })).toBeVisible();
  await expect(capsuleScience.getByTestId("capsule-visual")).toBeVisible();
