...
- 第二至第八屏都必须与前一屏发生可见的滚动覆盖，不得只用普通区块首尾相接。
- 覆盖随浏览器原生滚动发生：下一屏上沿先进入视口并压在上一屏之上，随后完整内容自然出现。
- 每个页面板使用递增层级，滚出视口后正常离开；不采用长时间 `sticky` 堆叠。
- 桌面、平板和手机保持同一覆盖顺序，只收敛圆角与覆盖距离。
- 第八屏先完成项目路径页面板到询盘页面板的覆盖，再进入既有 VITHELO 字标揭示；两段动效不得同时争抢注意力。

| 转场 | 桌面覆盖 | 平板覆盖 | 手机覆盖 | 视觉强度 |
| --- | ---: | ---: | ---: | --- |
| Hero → 制造系统 | 40px | 32px | 20px | 强，建立第一次翻页感 |
| 制造系统 → 合作起点 | 32px | 24px | 16px | 中，承接采购判断 |
| 合作起点 → 产品定义 | 28px | 22px | 14px | 中低，进入产品决策 |
| 产品定义 → 产品方向 | 24px | 20px | 12px | 低，以场景变化为主 |
| 产品方向 → 产品剂型 | 24px | 20px | 12px | 低，保持信息稳定 |
| 产品剂型 → 项目路径 | 32px | 24px | 16px | 中，形成执行章节停顿 |
| 项目路径 → 询盘 | 32px | 24px | 16px | 中，交给字标揭示收口 |

页面板上沿圆角统一为桌面 40px、平板 32px、手机 20px。覆盖距离可以变化，圆角语言不得逐屏变化，避免形成七套不同的卡片样式。
...
- 第二至第八屏均与前一屏形成可见滚动覆盖，覆盖距离符合桌面、平板和手机分级表，且不存在长时间吸顶堆叠。
