...
- 导航、八屏既有屏高以及各屏上下/左右内间距为当前几何锁定基线；首页叙事与动效调整不得改变这些值。
- 第五屏以“日常使用场景 → 产品简报”为叙事任务，使用与第三屏不同的场景图；保持原生纵向滚动，一次只显示一个方向，不显示页码、左右箭头，也不改成横向轮播。
...
- 第八屏询盘收口已实现 VITHELO 字标遮罩揭示软糖场景图，并在约 72% 进度完成，使 Email 与 WhatsApp 在约 75% 进度前清晰可用。滚动仍由浏览器原生控制，图片标记为 `DEMO_ONLY`。
...
- 完整单元测试当前为 126 项通过、0 项失败。
- 本轮首页叙事与动效定向 E2E 在 6 个验收视口为 38 项通过、10 项按视口条件跳过；覆盖第五屏单场景切换、第六屏响应式格式墙、第八屏提前完成、直达、键盘、无 JavaScript 与 Reduced Motion。
...
1. 由用户完成本轮首页叙事与动效视觉验收；在确认前不推送或部署。
2. 以当前八屏首页和几何契约作为锁定基线，后续只做用户明确提出的单屏定向修改。
