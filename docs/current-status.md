...
- `/oem-odm` 已按图片主导的十二段制造叙事完成重构，不采用 OEM / ODM 双路径对照，也不再使用能力卡片墙或表格式分隔线。
- 页面现包含大幅制造 Hero、定制开发总述、三组交替图文、八种剂型实物图、MOQ / 交期图文、质量场景、十步项目流程、白标包装场景、报价准备、包装方向、FAQ 与询盘收口。
- 新增白标包装静物图为 OpenAI 生成的 `DEMO_ONLY` 概念资产；不含品牌、功效文案或认证标记。所有制造内容继续保持条件化表达。
- 本轮 OEM / ODM 定向单元测试为 8 项通过，六个目标视口加 Reduced Motion 的定向 E2E 为 7 项通过，相关源文件定向 ESLint、全量类型检查和生产构建通过；桌面 1212 × 1267 与手机 390 × 844 已完成页面截图检查且无表格化视觉或横向溢出。
- 本地验证环境为 Node 24.16.0，不作为 Hostinger Node 20 生产证明；最终视觉接受仍以用户确认结果为准。
